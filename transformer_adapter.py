...
class ARTransformer(pl.LightningModule):
    ...
    def training_step(self, batch, batch_id):
        ...
        # print(batch_id, tim.shape, con.shape, dec_input.shape, dec_output.shape, src_lengths, tgt_lengths)
        
        # if (torch.isnan(tim).any() or torch.isinf(tim).any() or
        #     torch.isnan(con).any() or torch.isinf(con).any() or
        #     torch.isnan(dec_input).any() or torch.isinf(dec_input).any() or 
        #     torch.isnan(dec_output).any() or torch.isinf(dec_output).any()
        #     ):
            
        #     raise ValueError(f"tensor contains values out of bounds")
    ...
    def validation_step(self, batch, batch_id):
        tim, con, dec_input, dec_output, src_lengths, tgt_lengths = batch["tim"], batch["con"], batch["dec_input"], batch["dec_output"], batch["src_lengths"], batch["tgt_lengths"]
        # print(batch_id, tim.shape, con.shape, dec_input.shape, dec_output.shape, src_lengths, tgt_lengths)
        # print(batch_id, tim.device, con.device, dec_input.device, dec_output.device, src_lengths.device, tgt_lengths.device)
        # torch.Size([4, 337, 512]) torch.Size([4, 337, 1024]) torch.Size([4, 508]) torch.Size([4, 508]) tensor([326, 326, 326, 337], device='cuda:0') tensor([492, 492, 492, 508], device='cuda:0')
